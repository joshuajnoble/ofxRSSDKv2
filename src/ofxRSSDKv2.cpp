...
#pragma region Enable
	...
	bool RSDevice::enableHandTracking()
	{
		if (mSenseMgr)
		{
			if (mSenseMgr->EnableHand() >= PXC_STATUS_NO_ERROR)
			{
				//mHand = mSenseMgr->EnableHand();
				mHandTracker = mSenseMgr->QueryHand();
				if (mHandTracker)
				{

					mHandData = mHandTracker->CreateOutput();

					PXCHandConfiguration* config = mHandTracker->CreateActiveConfiguration();
					...
					mShouldGetHands = true;
				}
				else
				{
					mShouldGetHands = false;
				}
				return mShouldGetHands;
			}
			return false;
		}
		return false;
...
#pragma region Update
	...
	void RSDevice::updateHands()
	{
		...
		static pxcCHAR arr[1000]; // new

		mHandData->Update();

		//for (pxcI32 i = 0; i < mHandData->QueryNumberOfHands(); i++)
		//{
		//	PXCHandData::IHand* handData;
		//	if (mHandData->QueryHandData(PXCHandData::AccessOrderType::ACCESS_ORDER_BY_TIME, i, handData) == PXC_STATUS_NO_ERROR)
		//	{
